...
describe('Input', () => {
  ...
  it('支持 v-model', async () => {
    ...
    /*获取测试过程中组件触发的 'input' 和 'change' 事件的信息
将这些事件信息分别存储在 inputEvent 和 changeEvent 变量中
这些变量可以用于后续的测试断言，验证事件是否按预期触发且参数是否正确*/
  ...
  it('支持点击清空字符串', async () => {
    ...
    await input.trigger('blur') //触发失去焦点事件
    expect(wrapper.emitted()).toHaveProperty('blur') //断言出现了blur事件
  })
  it('支持切换密码显示', async () => {
    const wrapper = mount(Input, {
      props: {
